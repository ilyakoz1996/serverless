"use client";

import { Cross2Icon } from "@radix-ui/react-icons";
import { Table } from "@tanstack/react-table";

import { Button } from "@/components/ui/button";
import { DataTableViewOptions } from "./data-table-view-options";

import { invoiceStatus } from "./data/data";
import { DataTableFacetedFilter } from "./data-table-faceted-filter";
import { useGetProductsByProjectId } from "@/core/hooks/products";
import LStorage from "@/core/localStorage";
import { DataTableFacetedProductsFilter } from "./data-table-faceted-products-filter";

interface DataTableToolbarProps<TData> {
  table: Table<TData>;
}

export function DataTableToolbar<TData>({
  table,
}: DataTableToolbarProps<TData>) {

  const isFiltered = table.getState().columnFilters.length > 0;
  const storage = new LStorage()
  const project = storage.projects.getProject()
  const {data: products} = useGetProductsByProjectId(project?.id!)

  return (
    <div className="flex items-center justify-between">
      <div className="flex flex-1 items-center space-x-2">
        {table.getColumn("productId") && (
          <DataTableFacetedProductsFilter
            column={table.getColumn("productId")}
            title="Product"
            options={products! as any[]}
          />
        )}
        {table.getColumn("status") && (
          <DataTableFacetedFilter
            column={table.getColumn("status")}
            title="Status"
            options={invoiceStatus}
          />
        )}
        {isFiltered && (
          <Button
            variant="ghost"
            onClick={() => table.resetColumnFilters()}
            className="h-8 px-2 lg:px-3"
          >
            Reset
            <Cross2Icon className="ml-2 h-4 w-4" />
          </Button>
        )}
      </div>
      <DataTableViewOptions table={table} />
    </div>
  );
}
